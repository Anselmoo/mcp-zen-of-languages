...
def test_go_error_handling_detector():
    ...
    violations = run_detector(
        GoErrorHandlingDetector(), code, config_for("go_error_handling")
    )
    assert violations
...
def test_go_context_usage_detector():
    code = "package main\nfunc Work() {}"
    violations = run_detector(
        GoContextUsageDetector(), code, config_for("go_context_usage")
    )
    assert violations
...
def test_go_defer_usage_detector():
    ...
    violations = run_detector(
        GoDeferUsageDetector(), code, config_for("go_defer_usage")
    )
    assert violations
...
def test_go_naming_convention_detector():
    ...
    violations = run_detector(
        GoNamingConventionDetector(), code, config_for("go_naming_convention")
    )
    assert violations
...
def test_go_goroutine_leak_detector_flags_unclosed_channels():
    ...
    violations = run_detector(
        GoGoroutineLeakDetector(), code, config_for("go_goroutine_leaks")
    )
    assert violations
