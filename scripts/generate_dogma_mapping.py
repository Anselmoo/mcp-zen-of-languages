...
TESTING_DOGMA_SPEC_PATH = Path("docs/user-guide/rules/testing-dogmas.md")
...
_TESTING_TACTICS_LABELS: dict[TestingTacticsDogmaID, str] = {
    TestingTacticsDogmaID.ISOLATED: "Isolated",
    TestingTacticsDogmaID.DETERMINISTIC: "Deterministic",
    TestingTacticsDogmaID.SINGLE_REASON: "Single Reason",
    TestingTacticsDogmaID.NAMED_BEHAVIOR: "Named Behavior",
    TestingTacticsDogmaID.NO_LOGIC: "No Logic",
    TestingTacticsDogmaID.FAST: "Fast",
    TestingTacticsDogmaID.DOCUMENTED_INTENT: "Documented Intent",
    TestingTacticsDogmaID.TRUSTWORTHY: "Trustworthy",
    TestingTacticsDogmaID.CLEAN_CODE: "Clean Code",
    TestingTacticsDogmaID.PROPORTIONAL: "Proportional",
}

_TESTING_TACTICS_DETAIL: dict[TestingTacticsDogmaID, tuple[str, str, list[str]]] = {
    TestingTacticsDogmaID.ISOLATED: (
        "Each test stands alone — no shared mutable state, no execution-order dependency.",
        "Tests that depend on global state or previous tests become fragile and"
        " non-deterministic. Isolation guarantees that a single test failure points to"
        " a single root cause.",
        [
    ...
    TestingTacticsDogmaID.DETERMINISTIC: (
        "Given the same inputs, a test always produces the same result.",
        "Non-deterministic tests erode trust. Flaky tests that sometimes pass and"
        " sometimes fail are worse than no tests — they mask real failures and waste"
        " debugging time.",
        [
    ...
    TestingTacticsDogmaID.SINGLE_REASON: (
        "One test, one behavior — verify exactly one thing.",
        "Tests that check multiple behaviors require multiple code paths to fail before"
        " the root cause is clear. A test with a single assertion is trivially debuggable.",
        [
    ...
    TestingTacticsDogmaID.NAMED_BEHAVIOR: (
        "Test names document behavior, not implementation.",
        "A test named `test_1` or `should work` provides no signal on failure."
        " Names should read as specifications: what scenario, what expected outcome.",
        [
    ...
    TestingTacticsDogmaID.NO_LOGIC: (
        "Tests must not contain loops, conditionals, or complex logic.",
        "Logic inside a test means the test itself can be buggy. A test is a"
        " specification — it should read like a table of inputs and outputs, not a program.",
        [
    ...
    TestingTacticsDogmaID.FAST: (
        "Tests must not block on I/O, sleep, or real network calls.",
        "Slow tests discourage frequent runs. The test suite must complete in seconds,"
        " not minutes, to support TDD and CI gating.",
        [
    ...
    TestingTacticsDogmaID.DOCUMENTED_INTENT: (
        "Every non-obvious test decision must be explained in a comment or name.",
        "Future maintainers should understand why a test exists and what edge case it"
        " covers. Inline comments turn tests into living documentation.",
        [
    ...
    TestingTacticsDogmaID.TRUSTWORTHY: (
        "A passing test must mean the behavior is correct.",
        "Tests that always pass regardless of the implementation are worse than no"
        " tests. Assertions must be meaningful and cover the actual contract.",
        [
    ...
    TestingTacticsDogmaID.CLEAN_CODE: (
        "Tests follow the same code-quality standards as production code.",
        "Test code is read and maintained as often as production code. Dead code,"
        " magic numbers, and bare excepts are just as harmful in tests.",
        [
    ...
    TestingTacticsDogmaID.PROPORTIONAL: (
        "Test complexity must match the complexity of the code under test.",
        "Over-testing trivial code and under-testing complex logic are symmetric"
        " failures. Proportional coverage focuses effort where bugs are most likely.",
        [
...
_TESTING_STRATEGY_LABELS: dict[TestingStrategyDogmaID, str] = {
    TestingStrategyDogmaID.BOUNDARY: "Boundary",
    TestingStrategyDogmaID.TRACEABILITY: "Traceability",
    TestingStrategyDogmaID.INTEGRATION: "Integration",
    TestingStrategyDogmaID.RISK: "Risk",
    TestingStrategyDogmaID.REALITY_CHECK: "Reality Check",
    TestingStrategyDogmaID.FLAKINESS: "Flakiness",
    TestingStrategyDogmaID.SHIFT_RIGHT: "Shift Right",
    TestingStrategyDogmaID.OWNERSHIP: "Ownership",
    TestingStrategyDogmaID.EVOLVABILITY: "Evolvability",
    TestingStrategyDogmaID.VISIBILITY: "Visibility",
}

_TESTING_STRATEGY_DETAIL: dict[TestingStrategyDogmaID, tuple[str, str, list[str]]] = {
    TestingStrategyDogmaID.BOUNDARY: (
        "Test the edges, not just the happy path.",
        "Most production bugs live at boundaries: empty inputs, max values, type"
        " transitions, null-like sentinels. A strategy that covers only the happy path"
        " misses the majority of defect surfaces.",
        [
    ...
    TestingStrategyDogmaID.TRACEABILITY: (
        "Every test traces back to a requirement or acceptance criterion.",
        "Untraceable tests are orphans — they may test the wrong thing or duplicate"
        " coverage without anyone knowing. Traceability enables confident deletion when"
        " requirements change.",
        [
    ...
    TestingStrategyDogmaID.INTEGRATION: (
        "Test interactions between components, not just components in isolation.",
        "Unit tests validate contracts; integration tests validate that contracts are"
        " honored in composition. Both layers are necessary — neither replaces the other.",
        [
    ...
    TestingStrategyDogmaID.RISK: (
        "Allocate testing effort in proportion to risk, not uniformly.",
        "Not all code is equally critical. Risk-based testing focuses coverage on"
        " high-impact, frequently changing, or historically buggy modules.",
        [
    ...
    TestingStrategyDogmaID.REALITY_CHECK: (
        "Tests must reflect production conditions, not idealized lab conditions.",
        "Tests that only work under perfect conditions miss the bugs users actually"
        " encounter. Chaos, concurrency, and resource constraints must be modeled.",
        [
    ...
    TestingStrategyDogmaID.FLAKINESS: (
        "Flaky tests must be fixed or deleted — never silently retried.",
        "A test suite that auto-retries flaky tests hides non-determinism."
        " Every flaky test is a bug report waiting to be written.",
        [
    ...
    TestingStrategyDogmaID.SHIFT_RIGHT: (
        "Test in production-like environments as early as possible.",
        "The later a defect is found, the more expensive it is to fix. Shift-right"
        " testing (canary, shadow, production monitoring) catches classes of bugs that"
        " pre-production environments cannot.",
        [
    ...
    TestingStrategyDogmaID.OWNERSHIP: (
        "Tests are owned by the team that owns the code.",
        "Shared test suites with no clear owners accumulate technical debt. Ownership"
        " means the owning team fixes failures, updates tests for new requirements, and"
        " deletes obsolete coverage.",
        [
    ...
    TestingStrategyDogmaID.EVOLVABILITY: (
        "Tests must be refactored alongside production code.",
        "Tests that are never refactored become brittle. When a test suite impedes"
        " refactoring because every change breaks dozens of tests, the tests have"
    ...
    TestingStrategyDogmaID.VISIBILITY: (
        "Test results, coverage trends, and flakiness metrics must be visible to all.",
        "Invisible test quality leads to invisible quality debt. Dashboards, CI badges,"
        " and coverage trends make quality a team concern, not an individual one.",
        [
...
_TESTING_DOGMA_SPEC_FRONTMATTER = """\
---
...
def render_testing_dogma_spec() -> str:
    """Render the testing dogma spec page for docs/user-guide/rules/testing-dogmas.md."""
    lines: list[str] = [_TESTING_DOGMA_SPEC_FRONTMATTER, _TESTING_DOGMA_SPEC_INTRO]

    for i, dogma in enumerate(TestingTacticsDogmaID, start=1):
        label = _TESTING_TACTICS_LABELS[dogma]
        quote, rationale, anti_patterns = _TESTING_TACTICS_DETAIL[dogma]
        lines.append(f"### {i}. Dogma of {label} — `{dogma.value}`")
        lines.append("")
        lines.append(f"> {quote}")
        lines.append("")
    ...
    lines.append("## Macro Dogmas — `ZEN-MACRO-*`")
    lines.append("")
    lines.append(
        "Macro dogmas capture test *strategy* concerns that transcend individual"
        " test files. They are used in structural and dependency analysis."
    )
    lines.append("")

    for i, dogma in enumerate(TestingStrategyDogmaID, start=1):
        label = _TESTING_STRATEGY_LABELS[dogma]
        quote, rationale, anti_patterns = _TESTING_STRATEGY_DETAIL[dogma]
        lines.append(f"### {i}. Dogma of {label} — `{dogma.value}`")
        lines.append("")
        lines.append(f"> {quote}")
        lines.append("")
    ...
    lines.append("")
    lines.append(
        "- [The Ten Dogmas](the-ten-dogmas.md)"
        " — the universal cross-language dogmas these extend"
    )
    lines.append(
        "- [Understanding Violations](../understanding-violations.md)"
        " — severity scores and the MCP workflow"
    )
    lines.append("- [Languages](../languages/index.md) — per-language principles")
...
def main() -> int:
    ...
    expected_outputs = {
        ...
        TESTING_DOGMA_SPEC_PATH: render_testing_dogma_spec(),
    }
