...
`zen init --mcp-target vscode` can generate `.vscode/mcp.json` for you; keep the server key as `zen-of-languages`.
...
Codex reads MCP entries from the global config file rather than a workspace `.json` file. `zen init --mcp-target codex` can append this block for you. If you want to pin a repo-specific config file, add `env = { ZEN_CONFIG_PATH = "/absolute/path/to/zen-config.yaml" }` under the same table.

## GitHub Copilot-compatible JSON configs

`zen init` can also scaffold Copilot-compatible JSON files:
...
Both use the standard `mcpServers` JSON shape with the `zen-of-languages` server entry.
