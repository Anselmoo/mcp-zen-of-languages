...
def test_other_language_detectors_cover_paths():
    ...
    assert BashArgumentValidationDetector().detect(
        bash_args, BashArgumentValidationConfig()
    )
    ...
    assert CSharpStringInterpolationDetector().detect(
        csharp_str, CSharpStringInterpolationConfig()
    )
    ...
    go_loop = AnalysisContext(
        code="for i := 0; i < 1; i++ { defer foo() }", language="go"
    )
    ...
    js_context = AnalysisContext(
        code="function foo() { var x = 1; }", language="javascript"
    )
    ...
    assert JsCallbackNestingDetector().detect(
        AnalysisContext(
            code="function foo() { function bar() {} }", language="javascript"
        ),
        callback_cfg,
    )
    assert JsAsyncErrorHandlingDetector().detect(
        AnalysisContext(
            code="async function foo() { return 1 }", language="javascript"
        ),
        JsAsyncErrorHandlingConfig(),
    )
    ...
    assert PowerShellApprovedVerbDetector().detect(
        ps_context, PowerShellApprovedVerbConfig()
    )
    assert PowerShellErrorHandlingDetector().detect(
        ps_context, PowerShellErrorHandlingConfig()
    )
    assert PowerShellPascalCaseDetector().detect(
        ps_context, PowerShellPascalCaseConfig()
    )
    ...
    assert RubyNamingConventionDetector().detect(
        ruby_context, RubyNamingConventionConfig()
    )
    ...
    assert RustUnsafeBlocksDetector().detect(
        AnalysisContext(code="unsafe { }", language="rust"), RustUnsafeBlocksConfig()
    )
    assert RustCloneOverheadDetector().detect(
        AnalysisContext(code="x.clone();", language="rust"), RustCloneOverheadConfig()
    )
    ...
    ts_interface = AnalysisContext(
        code="type Foo = { a: string };", language="typescript"
    )
    assert TsInterfacePreferenceDetector().detect(
        ts_interface, TsInterfacePreferenceConfig()
    )
    ts_return = AnalysisContext(
        code="export function foo() { return 1 }", language="typescript"
    )
