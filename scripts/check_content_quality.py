"""Pre-commit hook: enforce strategic content-quality sections.

Checks:
1. Key README/docs pages include required strategic headings.
2. docs/index.md uses shared README snippets for MCP/philosophy parity.
3. Generated technical-reference pages retain auto-generated markers.
4. docs/index.md grid-card blocks keep expected opening/closing structure.
"""

from __future__ import annotations

import re
import sys

from pathlib import Path


REQUIRED_HEADINGS: dict[Path, tuple[str, ...]] = {
    Path("README.md"): (
        "## Why MCP for Zen Analysis",
        "## Zen Philosophy",
    ),
    Path("docs/index.md"): (
        "## Why MCP",
        "## Zen Philosophy",
        "## Choose Your Path",
    ),
    Path("docs/contributing/content-quality.md"): (
        "## Source-of-truth map",
        "## Editorial standards",
        "## Release checklist",
    ),
}

REQUIRED_SNIPPETS: dict[Path, tuple[str, ...]] = {
    Path("docs/index.md"): (
        '--8<-- "README.md:why-mcp"',
        '--8<-- "README.md:zen-philosophy"',
        '--8<-- "README.md:what-you-get"',
    ),
}

GENERATED_REFERENCE_MARKERS: dict[Path, tuple[str, ...]] = {
    Path("docs/user-guide/cli-reference.md"): ("Auto-generated", "cli.py"),
    Path("docs/user-guide/mcp-tools-reference.md"): ("Auto-generated", "server.py"),
    Path("docs/config.md"): ("Generated by", "scripts/generate_config_docs.py"),
}

INDEX_GRID_CARD_BLOCK_COUNTS: dict[Path, int] = {
    Path("docs/index.md"): 3,
}

GRID_CARD_BLOCK_OPENING = '<div class="grid cards" markdown>'
GRID_CARD_BLOCK_PATTERN = re.compile(
    r'<div class="grid cards" markdown>\n.*?\n</div>',
    re.DOTALL,
)

FORBIDDEN_STALE_CLAIMS: dict[Path, tuple[str, ...]] = {
    Path("README.md"): (
        "151 zen principles",
        "163 focused detectors",
        "for 14 languages into machine-readable rules",
    ),
    Path("docs/index.md"): (
        "151 zen principles across 14 languages",
        "Explore 14 languages",
    ),
    Path("docs/user-guide/index.md"): ("14 languages, 151 principles, 163 detectors",),
    Path("docs/contributing/architecture.md"): ("163 detectors across 14 languages",),
    Path("docs/overrides/main.html"): ("14+ languages",),
}


def _read(path: Path, errors: list[str]) -> str:
    if not path.exists():
        errors.append(f"{path}: file not found")
        return ""
    return path.read_text(encoding="utf-8")


def _check_required_headings(errors: list[str]) -> None:
    for path, headings in REQUIRED_HEADINGS.items():
        text = _read(path, errors)
        if not text:
            continue
        errors.extend(
            f"{path}: missing required heading '{heading}'"
            for heading in headings
            if heading not in text
        )


def _check_required_snippets(errors: list[str]) -> None:
    for path, snippets in REQUIRED_SNIPPETS.items():
        text = _read(path, errors)
        if not text:
            continue
        errors.extend(
            f"{path}: missing required snippet include '{snippet}'"
            for snippet in snippets
            if snippet not in text
        )


def _check_generated_markers(errors: list[str]) -> None:
    for path, markers in GENERATED_REFERENCE_MARKERS.items():
        text = _read(path, errors)
        if not text:
            continue
        errors.extend(
            f"{path}: missing generated-doc marker '{marker}'"
            for marker in markers
            if marker not in text
        )


def _check_index_grid_card_blocks(errors: list[str]) -> None:
    for path, expected_count in INDEX_GRID_CARD_BLOCK_COUNTS.items():
        text = _read(path, errors)
        if not text:
            continue
        opening_count = text.count(GRID_CARD_BLOCK_OPENING)
        if opening_count != expected_count:
            errors.append(
                f"{path}: expected {expected_count} grid card openings, found {opening_count}"
            )
        closed_block_count = len(GRID_CARD_BLOCK_PATTERN.findall(text))
        if closed_block_count != expected_count:
            errors.append(
                f"{path}: expected {expected_count} complete grid card blocks, found {closed_block_count}"
            )


def _check_forbidden_stale_claims(errors: list[str]) -> None:
    for path, phrases in FORBIDDEN_STALE_CLAIMS.items():
        text = _read(path, errors)
        if not text:
            continue
        errors.extend(
            f"{path}: stale claim found '{phrase}'"
            for phrase in phrases
            if phrase in text
        )


def main() -> int:
    errors: list[str] = []

    _check_required_headings(errors)
    _check_required_snippets(errors)
    _check_generated_markers(errors)
    _check_index_grid_card_blocks(errors)
    _check_forbidden_stale_claims(errors)

    if errors:
        print("❌ Content quality checks failed:")
        for error in errors:
            print(f"  • {error}")
        return 1

    print("✅ Content quality checks passed")
    return 0


if __name__ == "__main__":
    sys.exit(main())
